...
p1 <- data %>% 
  ...
  geom_histogram(bins = 15) + labs(title = "Freedom Index", subtitle = 'Further Right More "Free"')

p2 <- data %>% 
  ggplot(mapping = aes(x = -polrights_fh)) + 
  geom_histogram(bins = 15) + labs(title = "Political Rights Index")

p3 <- data %>% 
  ggplot(mapping = aes(x = -cliberities_fh)) + 
  geom_histogram(bins = 15) + labs(title = "Civil Liberties Index")
...
Best subsets resuls show most vars are insignificant. Can we procede with the project?
